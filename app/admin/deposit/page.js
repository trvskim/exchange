...
export default async function List(req, res) {
  ...
  if ((paramsData.search == "") | !paramsData.search) {
    option = "email";
    search = { $regex: /@/, $options: "i" };
  } else if (paramsData.option == "holder") {
    option = "name";
    search = paramsData.search;
  } else if (paramsData.option == "order-id") {
    option = "_id";
    try {
      search = ObjectId(paramsData.search);
    } catch (err) {
      search = "";
    }
  } else if (paramsData.option == "email") {
    option = "email";
    search = paramsData.search;
  }
